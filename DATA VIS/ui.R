...
dashboardPage(
  ...
  dashboardHeader(
    title = "Evolution du prix domestique de gaz naturel et d'électricité",
    titleWidth = 800,
    tags$li(
      class = "dropdown",
      tags$a(
        href = "https://github.com/Liora181002/Data-vis", 
        target = "_blank",  # Correct placement of target attribute
        icon("github"), 
        "Code source"
      )
    )
  ...
  dashboardSidebar(
    ...
    sidebarMenu(
      ...
      menuItem(text = "Visualisation", tabName = "viz", icon = icon("chart-line"), badgeLabel = "Cliquez pour voir", badgeColor = "blue"),
                     ...
                     tabPanel("A propos de", icon = icon("address-card"),
                              fluidRow(
                                ...
                                column(width = 4, tags$br(),
                                       tags$p("Cette base de données recense les prix de l'électricité et du gaz en €/MWh en France et dans l'Union Européenne entre 1995 et 2021, avec une fréquence d'enregistrement biannuelle (deux fois par an). Il convient de noter que le gaz, étant moins efficace énergétiquement que l'électricité, affiche logiquement un prix au MWh nettement inférieur.

Nous visualiserons ces données à l'aide de différentes méthodes de représentation graphique pour mieux comprendre les évolutions constatées. Ensuite, nous nous concentrerons sur le prix du gaz en €/MWh entre 2011 et 2022 dans les pays de l'UE, que nous représenterons sur une carte de l'Europe. Cela permettra d'examiner l'influence potentielle de facteurs géographiques sur le prix du gaz dans l'Union Européenne.."))
                              )
                     ),
